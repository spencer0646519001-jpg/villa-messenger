...
def make_tenant_amenities_loader(database_path: str | Path, base_dir: str | Path = "data/tenants") -> Callable[[int], dict]:
    def load(tenant_id: int) -> dict:
        return _load_block(database_path, tenant_id, "amenities", base_dir)
    return load


def make_tenant_room_policy_loader(database_path: str | Path, base_dir: str | Path = "data/tenants") -> Callable[[int], dict]:
    def load(tenant_id: int) -> dict:
        return _load_block(database_path, tenant_id, "room_policy", base_dir)
    return load


def make_tenant_location_loader(database_path: str | Path, base_dir: str | Path = "data/tenants") -> Callable[[int], dict]:
    def load(tenant_id: int) -> dict:
        return _load_block(database_path, tenant_id, "location", base_dir)
    return load

...
- **白板本身還沒填滿完整資訊時,不算「重講」**(Codex review of `28f67f7`,P1)——
  原本只比較這輪 `adult_count` 跟白板上 `adult_count` 是否不同,漏掉了白板根本
  還沒有完整日期範圍、或還沒問到 `adult_count` 的情況。例如白板已經有「10人、
  3房、養1隻狗、要烤肉」但還沒給日期,這輪第一次補上日期並順便把人數講清楚成
  「8大2小」——舊寫法會把這個「補資料」動作誤判成「換了一群人」,換白板後把
  房數/寵物/烤肉這些已經問到的資訊全部沖掉。改成先要求白板本身「已經有完整日期
  範圍」且「已經有 `adult_count`」,兩者都成立才有資格判斷「重講」;純粹補資料
  (白板缺角)一律走原地更新。
- **expire 舊狀態 + insert 新狀態改成同一個 SQLite transaction**(Codex review of
  `28f67f7`,P2)——原本是兩次獨立呼叫(`mark_expired()` 再 `create()`),中間任何
  一步失敗(例如 insert 因為 unique index 衝突丟例外),舊狀態已經被標記過期但
  新狀態沒建立成功,客人這輪累積的資料就整個不見。新增
  `ConversationStateRepository.supersede()`,在同一個 connection 裡先 UPDATE
  舊狀態、再 INSERT 新狀態,最後才 commit;insert 失敗時整個 transaction 不會
  commit,舊狀態的 `in_progress` 狀態保持不變。
...
3. **3-B**:採用「換一塊新白板」而非原提案的「原地清空」(理由見 3-B 節)。
   ...
   都已經反映進最終的觸發條件。control_11 通過。實作 commit `28f67f7`,Codex
   review 抓到兩個邊界案例(白板還沒填滿完整資訊時誤判成重講的 P1、expire+create
   非原子操作可能丟資料的 P2),都已修正並補上對應 regression test。
